...
export default function AdminLiveClassesPage() {
  ...
  const handleStart = async (lc: LiveClass) => {
    const res = await fetch(`/api/live-classes/${lc.id}/start`, {
      method: 'POST',
    })
    if (res.ok) {
      window.location.href = `/admin/lms/live-classes/${lc.id}/room`
    } else {
      const data = await res.json()
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <Link
              href="/admin"
              className="px-5 py-2.5 bg-slate-800 text-slate-300 rounded-xl text-sm font-semibold hover:bg-slate-700 transition"
            >
              ← Admin
            </Link>
            <Link
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span
                        className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${statusColor(
                          lc.status
                        )}`}
                      >
                        {lc.status}
                      ...
                      {!lc.is_published && (
                        <span className="px-2 py-1 rounded-md bg-yellow-500/10 text-yellow-400 text-[10px] font-bold">
                          DRAFT
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {lc.status === 'SCHEDULED' && (
                      <button
                        ...
                        className="px-4 py-2 rounded-lg bg-red-600 text-sm font-medium text-white hover:bg-red-700 transition"
                      >
                        ● Start Class
                      </button>
                    )}
                    ...
                    {lc.status === 'LIVE' && (
                      <a
                        href={`/admin/lms/live-classes/${lc.id}/room`}
                        className="px-4 py-2 rounded-lg bg-red-500/20 text-sm font-medium text-red-400 hover:bg-red-500/30 transition animate-pulse"
                      >
                        ● Join Live
                      </a>
                    )}
